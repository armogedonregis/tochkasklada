  ...
  private async sendAllExpirationNotifications() {
    ...
    for (const rental of activeRentals) {
      try {
        ...
        if (shouldSend) {
          ...
          try {
            ...
            if (!success) {
              failedEmails++;
              this.logger.warn(
                `Failed to send email to ${rental.client.user.email} for rental ${rental.id}`,
                'EmailTasksService'
              );
            }
            ...
            failedEmails++;
            this.logger.error(
              `Error sending email to ${rental.client.user.email}: ${emailError.message}`,
              emailError.stack,
              'EmailTasksService'
            );
        ...
        failedEmails++;
        this.logger.error(
          `Error processing rental ${rental.id}: ${rentalError.message}`,
          rentalError.stack,
          'EmailTasksService'
        );
      }
    ...
    this.logger.log(
      `Email notifications completed: 7 days - ${sent7Days}, 3 days - ${sent3Days}, 1 day - ${sent1Day}, expired - ${sentExpired}, failed - ${failedEmails}`,
      'EmailTasksService'
    );
  }
}
