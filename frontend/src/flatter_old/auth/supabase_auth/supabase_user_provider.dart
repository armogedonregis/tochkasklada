...
class CrmtochkaSupabaseUser extends BaseAuthUser {
  CrmtochkaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> crmtochkaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CrmtochkaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
